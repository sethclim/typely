...
export class DBService {
	...
	async init() {
		...
		this._rawDB = savedData ? new this.SQL.Database(new Uint8Array(JSON.parse(savedData))) : new this.SQL.Database()
	...
	subscribe(table: string, cb: Listener) {
		...
		return () => {
			this.tableListeners[table] = this.tableListeners[table].filter((l) => l !== cb)
		}
	}
