...
export const DuplicateResume = async (data: Repositories, _svc: DBService, id: number) => {
	const resume_raw = await data.resumeConfig.getResumeConfig(id)
	...
	try {
		...
		const newResumeResult = await _svc.db
			?.insert(resumeConfig)
			.values({
				uuid,
		...
		_svc.notifyTable('resume_config')
		...
		for (const section of resume.sections) {
			...
			// Insert section
			const newSectionResult = await _svc.db
			...
			if (newSectionResult == undefined) throw Error('newSectionResult UNDEFINED')
			...
			_svc.notifyTable('resume_section_data')
		}
	} catch (e) {
