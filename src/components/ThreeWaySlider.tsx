...
export default function ThreeWaySlider({ options, value, onChange, className = '' }: ThreeWaySliderProps) {
	...
	return (
		<RadioGroup value={value} onChange={onChange} className={className}>
			<div className="relative h-10 bg-dark flex items-center px-2">
				{/* Slider indicator */}
				<div
					className={clsx('absolute top-1 bottom-1 w-1/3 rounded-50 bg-darkest transition-all duration-200', {
						'left-1': index === 0,
						'left-1/3': index === 1,
						'left-2/3': index === 2
					})}
				...
				<div className="flex w-full h-full">
					{options.map((opt) => (
						...
						>
							{({ checked }) => (
								<span className={checked ? 'text-mywhite font-semibold' : 'text-grey'}>{opt}</span>
							)}
						</Radio>
					))}
				</div>
