...
export default function AutoThemeResumeDemo() {
	...
	return (
		<motion.div className={`min-h-[400px] flex items-center justify-center p-8 transition-colors`} layout>
			<motion.div
				layout
			...
			>
				{/* Header */}
				<motion.div layout className={`flex flex-col ${theme.headerLayout}`}>
				...
				{/* Divider */}
				<motion.hr layout className={`border-t-2 ${theme.divider}`} />
				...
				<motion.div layout className={theme.experienceLayout}>
					{resumeData.experience.map((exp) => (
						<div key={exp.role} className="flex justify-between gap-2">
							<span>
								<strong>{exp.role}</strong> {!theme.font.includes('mono') && `@ ${exp.company}`}
							</span>
