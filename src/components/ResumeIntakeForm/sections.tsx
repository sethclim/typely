...
export const SkillsSection = (props: SkillsSectionProps) => {
	return (
		<Section title="Skills">
					...
					<div className="grid grid-cols-4 gap-4">
						<div className="col-span-1">
							<Input
								...
								value={skill.title}
								onChange={(v: string) =>
									props.setSkillPoints((s) => s.map((x, idx) => (idx === i ? { ...x, title: v } : x)))
								}
							/>
						</div>
						...
						<div className="col-span-3">
							<Input
								...
								value={skill.skills}
								onChange={(v: string) =>
									props.setSkillPoints((j) =>
										j.map((x, idx) => (idx === i ? { ...x, skills: v } : x))
									)
								}
							/>
						</div>
			...
			<AddButton id="skills-add-btn" onClick={props.addSkill}>
				+ Skills
			</AddButton>
		</Section>
	)
		...
		<Section title="Experience">
			{props.jobs.map((job, i) => (
				<Card key={i} onClose={() => props.close(i)}>
					<Input
						input_id={`exp-company-input-${i}`}
						label="Company"
						value={job.company}
						onChange={(v: string) =>
							props.setJobs((j) => j.map((x, idx) => (idx === i ? { ...x, company: v } : x)))
						}
					/>
					<Input
						input_id={`exp-title-input-${i}`}
						label="Title"
						value={job.title}
						onChange={(v: string) =>
							props.setJobs((j) => j.map((x, idx) => (idx === i ? { ...x, title: v } : x)))
						}
					/>
					<div className="grid grid-cols-2 gap-4">
						<Input
							input_id={`exp-start-date-input-${i}`}
							label="Start Date"
							value={job.startDate}
							onChange={(v: string) =>
								props.setJobs((j) => j.map((x, idx) => (idx === i ? { ...x, startDate: v } : x)))
							}
						/>
						<Input
							input_id={`exp-end-date-input-${i}`}
							label="End Date"
							value={job.endDate}
							onChange={(v: string) =>
								props.setJobs((j) => j.map((x, idx) => (idx === i ? { ...x, endDate: v } : x)))
							}
						/>
					</div>
					<Input
						input_id={`exp-location-input-${i}`}
						label="Location"
						value={job.location}
						onChange={(v: string) =>
							props.setJobs((j) => j.map((x, idx) => (idx === i ? { ...x, location: v } : x)))
						}
					/>
					<Input
						input_id={`exp-point-one-input-${i}`}
						label="Point One"
						value={job.pointOne}
						onChange={(v: string) =>
							props.setJobs((j) => j.map((x, idx) => (idx === i ? { ...x, pointOne: v } : x)))
						}
					/>
					<Input
						input_id={`exp-point-two-input-${i}`}
						label="Point Two"
						value={job.pointTwo}
						onChange={(v: string) =>
							props.setJobs((j) => j.map((x, idx) => (idx === i ? { ...x, pointTwo: v } : x)))
						}
					/>
					<Input
						input_id={`exp-point-three-input-${i}`}
						label="Point Three"
						value={job.pointThree}
						onChange={(v: string) =>
							props.setJobs((j) => j.map((x, idx) => (idx === i ? { ...x, pointThree: v } : x)))
						}
					/>
					<Input
						input_id={`exp-point-four-input-${i}`}
						label="Point Four"
						value={job.pointFour}
						onChange={(v: string) =>
							props.setJobs((j) => j.map((x, idx) => (idx === i ? { ...x, pointFour: v } : x)))
						}
					/>
				</Card>
			))}

			<AddButton id="job-add-btn" onClick={props.addJob}>
				+ Add Job
			</AddButton>
		</Section>
	)
			...
			{props.projects.map((project, i) => (
				<Card key={i} onClose={() => props.close(i)}>
					<Input
						...
						value={project.title}
						onChange={(v: string) =>
							props.setProjects((p) => p.map((x, idx) => (idx === i ? { ...x, title: v } : x)))
						}
					/>
					...
					<Input
						...
						onChange={(v: string) =>
							props.setProjects((p) => p.map((x, idx) => (idx === i ? { ...x, pointOne: v } : x)))
						}
					/>
					...
					<Input
						...
						onChange={(v: string) =>
							props.setProjects((p) => p.map((x, idx) => (idx === i ? { ...x, pointTwo: v } : x)))
						}
					/>
					...
					<Input
						...
						onChange={(v: string) =>
							props.setProjects((p) => p.map((x, idx) => (idx === i ? { ...x, pointThree: v } : x)))
						}
					/>
					...
					<Input
						...
						onChange={(v: string) =>
							props.setProjects((p) => p.map((x, idx) => (idx === i ? { ...x, pointFour: v } : x)))
						}
					/>
				</Card>
			))}

			<AddButton id="project-add-btn" onClick={props.addProject}>
				+ Add Project
			</AddButton>
		</Section>
	)
			...
			{props.education.map((ed, i) => (
				<Card key={i} onClose={() => props.close(i)}>
					<Input
						...
						value={ed.school}
						onChange={(v: string) =>
							props.setEducation((e) => e.map((x, idx) => (idx === i ? { ...x, school: v } : x)))
						}
					/>
					...
					<Input
						...
						value={ed.program}
						onChange={(v: string) =>
							props.setEducation((e) => e.map((x, idx) => (idx === i ? { ...x, program: v } : x)))
						}
					/>
					...
					<Input
						...
						value={ed.location}
						onChange={(v: string) =>
							props.setEducation((e) => e.map((x, idx) => (idx === i ? { ...x, location: v } : x)))
						}
					/>
					...
					<div className="grid grid-cols-2 gap-4">
						<Input
							...
							value={ed.startDate}
							onChange={(v: string) =>
								props.setEducation((e) => e.map((x, idx) => (idx === i ? { ...x, startDate: v } : x)))
							}
						/>
						...
						<Input
							...
							value={ed.endDate}
							onChange={(v: string) =>
								props.setEducation((e) => e.map((x, idx) => (idx === i ? { ...x, endDate: v } : x)))
							}
						/>
					</div>
					...
					<Input
						...
						value={ed.gpa}
						onChange={(v: string) =>
							props.setEducation((e) => e.map((x, idx) => (idx === i ? { ...x, gpa: v } : x)))
						}
					/>
				</Card>
			))}

			<AddButton id="edu-add-btn" onClick={props.addEducation}>
				+ Add Education
			</AddButton>
		</Section>
	)
