				...
				<div className="text-mywhite min-h-80">
					...
					{selectedKeys.length > 0 ? (
						<div className="flex justify-end">
							<button className="text-mywhite bg-primary px-2 py-1 rounded-sm" onClick={onNextStage}>
								NEXT
							</button>
						</div>
					) : null}
			...
			{stage == 1 ? (
				<>
					<div className="flex flex-rwo justify-end">
						{!props.edit ? (
							<button className="text-darkest bg-grey px-2 py-1 rounded" onClick={() => setStage(0)}>
								Back
							</button>
						) : null}
					</div>
					<form>
						...
						<input
							className="text-mywhite bg-dark p-1"
							value={title}
						...
						<div className="flex flex-col border-solid border-black">
								...
								<tbody className="bg-white divide-y divide-white border border-grey">
									{items.map(([key, value], index) => (
										<tr key={index}>
											...
											<td>
												<input
													className="text-mywhite bg-dark p-1 w-full"
													value={key}
													onChange={(e) => updateItem(index, e.target.value, 0)}
												/>
											</td>
											{/* <p className="text-black px-2">Value</p> */}
											<td>
												<input
													className="text-mywhite bg-dark p-1 mr-2 w-full"
													value={value}
													onChange={(e) => updateItem(index, e.target.value, 1)}
												/>
											</td>
										</tr>
									))}
							...
							<div className="flex justify-end p-2 pt-4">
								<button className="bg-primary px-4 text-white rounded-sm" onClick={(e) => addItem(e)}>
									Add New Item
								</button>
							</div>
						</div>
