...
export const OutputView = (props: OutputViewProps) => {
	...
	useEffect(() => {
		const init = async () => {
			const { data: users } = await supabase.from('users').select('compile_count')

			if (users) setCompileCount(users[0].compile_count)
		}
		init()
	}, [pdfUrl])

	const downloadPDF = () => {
		if (pdfUrl === null || props.resume === undefined || props.resume === null) return
	...
	const downloadLatex = () => {
		if (level === null || props.resume === undefined || props.resume === null) return
	...
	const performTemplating = () => {
		...
		if (props.resume?.sections[1]) ReplaceVariables(props.resume?.sections[1])
	...
	return (
		<div className="p-4 h-full bg-black flex flex-col justify-start">
			<div className="flex justify-start">
				...
				<button className="bg-white m-2 p-1 px-2 font-bold rounded-sm" onClick={downloadPDF}>
					<div className="flex flex-row justify-between items-center">
						PDF
						<ArrowDownTrayIcon className="h-3 w-3 ml-2 text-black" />
					</div>
				</button>
			...
			<ThreeWaySlider options={['PDF', 'LATEX']} value={level} onChange={setLevel} className="w-full" />
			{level == 'PDF' ? (
				<>
					{!user ? (
						<div className="flex h-full w-full justify-center items-center">
							...
							>
								<div className="bg-purple-500 text-white p-2 rounded-sm">Login To Compile</div>
							</Link>
						</div>
						...
						<div className="flex flex-col gap-2 p-4  justify-center items-center bg-dark mt-8">
							...
							<Link to="/pricing" className="bg-primary p-2 text-mywhite w-50 rounded-sm">
								Buy Now
							</Link>
							<h3 className="text-grey">--------- OR ---------</h3>
							<span className="flex flex-row">
								<p className="text-white mr-2">See last compiled version:</p>
								<button className="text-blue-500" onClick={() => setShowLastCompiled(true)}>
									HERE
								</button>
							</span>
							...
							<div className="flex flex-col items-center">
								<p className="text-white">
									You can always download the latex and compile locally, or on overleaf etc.
								</p>
								<span className="flex flex-row">
									<p className="text-white mr-2">Please see our</p>
									<Link to="/faq" className="text-blue-500  mr-2">
										FAQ
									</Link>
