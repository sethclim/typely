...
export const ResumeView = () => {
	...
	const createResumeComponent = () => {
		if (myResume === null || selectedType == null || title == undefined) return
	...
	function handleDragEnd(event: DragEndEvent) {
		...
		if (overPrefix === 'dataitem' && activePrefix === 'dataitem') {
			...
			const match = over.data.current?.find((item: DataItem) => {
				...
				return (
					keys.length === active_data_item_keys.length && keys.every((k, i) => k === active_data_item_keys[i])
				)
			})

			if (match != undefined) {
				//show modal
				setReplaceDataItemData({
					section_id: section_id,
					active: active.data.current as unknown as DataItem,
					match: match
				})
	...
	const onSelectedTypeChange = (value: string | null) => {
		...
		if (filtered) setSelectedTemplate(filtered.name)
		else {
			setSelectedTemplate('')
		}
	}
			...
			<div className="flex flex-1 flex-row w-lvw justify-start bg-darkest p-4 gap-2">
				...
				<Modal isOpen={isNewResumeOpen} onClose={() => setIsNewRsumeOpen(false)} width="w-100">
					...
					<form>
						...
						<input
							className="bg-gray-200 w-64 p-1 my-2"
							value={title ?? ''}
						...
						<ComboBox
							...
							options={['Header', 'Skills', 'Experience', 'Project', 'Education']}
						/>
				...
				<Modal isOpen={isReplaceDataItemOpen} onClose={() => setReplaceDataItemOpen(false)} width="w-100">
					<h2 className="text-xl text-center font-bold mb-4 text-mywhite">
						{replaceDataItemData?.active.title} has same keys as {replaceDataItemData?.match.title}
					</h2>
					...
					<div className="flex justify-center flex-row gap-4">
						<button
							...
							onClick={() =>
								ReplaceDataItem(
									replaceDataItemData?.section_id!,
									replaceDataItemData?.match.id!,
									replaceDataItemData?.active.id!
								)
							}
						>
							Yes
						...
						<button
							className="mt-4 px-4 py-2 bg-dark text-white hover:bg-grey hover:text-darkest rounded"
							onClick={() =>
								AddDataItem(replaceDataItemData?.section_id!, replaceDataItemData?.active.id!)
							}
						>
							No Add Anyways
						</button>
